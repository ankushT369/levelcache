...
int main() {
    ...
    SplitCache* cache = splitcache_open(db_path, 10);
    if (cache == NULL) {
    ...
    if (splitcache_put(cache, key1, value1, 0) != 0) {
    ...
    if (splitcache_put(cache, key2, value2, 2) != 0) {
        fprintf(stderr, "Failed to put value.\n");
        splitcache_close(cache);
        return 1;
    }
    ...
    // Get the first value
    char* retrieved_value = splitcache_get(cache, key1);
    ...
    retrieved_value = splitcache_get(cache, key2);
    if (retrieved_value != NULL) {
